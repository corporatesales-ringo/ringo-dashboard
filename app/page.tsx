...
function ChannelCard({ label, revenue, orders, icon: Icon, percent }) {
  return (
    <Card className="rounded-2xl border-0 shadow-sm">
      <CardContent className="p-5">
        <div className="flex items-start justify-between gap-4">
...
export default function RevenueDashboardReact() {
  ...
  const [selectedChannels, setSelectedChannels] = useState(["Shopify", "Amazon", "Wholesale", "Retail"]);
  const [manualRows, setManualRows] = useState(manualSeed);
  const [form, setForm] = useState({ date: "", channel: "Wholesale", account: "", amount: "", notes: "" });
  ...
  const totalUnits = useMemo(
    () => channelRows.filter((r) => selectedChannels.includes(r.channel)).reduce((sum, r) => sum + r.units, 0),
    [selectedChannels]
  );
